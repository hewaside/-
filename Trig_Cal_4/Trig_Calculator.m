    ...
    switch input_type
        case '1'
            ...
            % 计算正弦、余弦和正切
            sin_val = calculateSin(rad,10 ,unit);
            cos_val = calculateCos(rad,10 ,unit);
            ...
            if mod(deg,90)==0 && mod(deg/90,2) ~= 0 
                fprintf('此时Tan不存在。\n');     
                 ...
                 tan_val = calculateTan(rad,10 ,unit);
                 fprintf('Tan: %.*f\n', input_unit,tan_val);
            end            
            % 不计算反正弦、反余弦和反正切
