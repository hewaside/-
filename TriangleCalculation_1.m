function TriangleCalculation_1()
    % 请求用户输入类型
    ...
    switch input_type
        case '1'
            ...
            % 计算正弦和余弦
            sin_val = calculateSin(rad, 100);
            cos_val = calculateCos(rad, 100);
...
function sin_val = calculateSin(rad, n_terms)
    sin_val = 0;
    for i = 0:n_terms
        sign = (-1)^(i);
...
function cos_val = calculateCos(rad, n_terms)
    cos_val = 0;
    for i = 0:n_terms
        sign = (-1)^(i);
        fact = factorial(2*i);
